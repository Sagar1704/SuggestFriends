		...
		if (profile.length == 2) {
			...
			for (int friend1 = 0; friend1 < friends.size(); friend1++) {
				for (int friend2 = friend1
						+ 1; friend2 < friends.size(); friend2++) {
					context.write(
							new LongWritable(friends.get(friend1)),
							new MutualFriendWritable(friends.get(friend2), userId));

					context.write(
							new LongWritable(friends.get(friend2)),
							new MutualFriendWritable(friends.get(friend1), userId));
				}
			}
		}
	}
